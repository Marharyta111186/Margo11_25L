...
public class Garden {
    ...
    name, height, age. Каждый из них вырастает на разную высоту за сезон, причем эта высота одинаковая для
    всех деревьев и всех цветов.
    Каждый год начинается весной и состоит из четырех времен года, во время которых поведние деревьев и цветов может
     отличатся:
     Цветы:
     ...
     осенью дереввья не растут
     зимой деревья не растут

     В классе Garden создать метод growPlants в котором должен быть отражен процесс роста нескольких растений в течении
     нескольких лет. Что происходит зимой, летом, весной осенью в каждый сезон и какого они роста и возраста в конце
